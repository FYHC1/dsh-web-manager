...
#    both (win) and (wsl) shortcuts use. The bundled exe is copied so the manager
#    supports the "open windows"/"open wsl" actions; the user's config/state live
#    under %USERPROFILE%\.dsh-webui and are never touched here.
...
if (-not (Test-Path -LiteralPath $bundled -PathType Leaf)) {
    ...
    $alreadyHad = Test-Path -LiteralPath $exe -PathType Leaf
    if ($alreadyHad) { Write-Host "[ensure-shortcut] reusing shared manager location: $appRoot" }
    ...
    if (-not $alreadyHad) { Write-Host "[ensure-shortcut] installed shared manager to $appRoot" }
}
